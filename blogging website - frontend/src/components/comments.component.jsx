...
const CommentsContainer = () => {
  const { blog, commentsWrapper, setCommentsWrapper, totalParentCommentsLoaded, setTotalParentCommentsLoaded, setBlog } = useContext(BlogContext);

  const { _id, title } = blog || {};
  const commentsArr = blog?.comments?.results || [];
  const { total_parent_comments } = blog?.activity || {};
  ...
  return (
    <div
      className={
        "fixed top-0 h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden duration-700 " +
        "max-sm:w-full w-[30%] min-w-[350px] " +
        (commentsWrapper ? "right-0" : "right-[-100%]")
      }>
