...
const emailRegex = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/; // Regex for email validation
...
// Format user data to send in response
    ...
    try {
        ...
        if (!passwordRegex.test(password)) {
            return res.status(400).json({
                error: "Password should be 6 to 20 characters long with a number, 1 lowercase, and 1 uppercase letter"
            });
        }
...
server.post("/google-auth", async (req, res) => {
    ...
    getAuth()
        ...
        .then(async (decodedUser) => {
            ...
            if (user) {//login
                if (!user.google_auth) {
                    return res.status(403).json({ "error": "This email was signed up without google. Plase log in with password to access the account" })
                }
            }
...
server.post('/change-password', verifyJWT, (req, res) => {
    ...
    if(!passwordRegex.test(currentPassword) || !passwordRegex.test(newPassword)){
            return res.status(403).json({ error :"Password should be 6 to 20 characters long with a number, 1 lowercase, and 1 uppercase letter" })
        }

        User.findOne({ _id: req.user })
        .then((user) =>  {
            if(user.google_auth){
                return res.status(403).json({error: " You can't change accounts password because you logged in through google "})
            }
...
server.post("/all-latest-blogs-count", (req, res) => {
    Blog.countDocuments({ draft: false })
        .then(count => {
            // Change key from total_Docs to totalDocs here:
...
server.post('/create-blog', verifyJWT, (req, res) => {
    ...
    if (!title.length) {
        return res.status(403).json({ error: "You must provide a title to Pubish the blog" });
    }
...
server.post("/add-comment", verifyJWT, async (req, res) => {
  try {
    ...
    // If reply, update parent comment and assign notification_for properly
    if (replying_to) {
      ...
      if (notification_id) {
        ...
        console.log("Notification updated with reply");
      }
    ...
    return res.status(500).json({ error: "Something went wrong while adding comment" });
  }
});
...
server.post("/get-replies", async (req, res) => {
    ...
    try {
        const parentComment = await Comment.findOne({ _id })
            .populate({
                path: "children",
                options: {
                    limit: maxLimit,
        ...
        // Inject replyToId manually to each reply
        const repliesWithParent = parentComment.children.map(reply => ({
            ...reply.toObject(),
            replyToId: _id,
        }));

        return res.status(200).json({ replies: repliesWithParent });
    ...
    Comment.findOneAndDelete({ _id })
        .then(comment => {
            ...
            if (comment.parent) {
                ...
                .then(() => console.log('Comment removed from parent'))
            ...
            // Update blog activity & comment list
            Blog.findOneAndUpdate(
                ...
                // 🔒 Only recursively delete if it's a parent comment
                if (!comment.parent && comment.children.length) {
